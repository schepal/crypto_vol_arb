...
class FTX:
     """
     The `FTX` class object is used to retrieve MOVE contract data which is
     ...
     def get_move_strike(self, ftx_contract_name):
        """
        Retrieves the strike price of the FTX MOVE Contract. In case there is no strike listed,
...
class VolArb(FTX):
     ...
     price differentials between FTX and Deribit.
     """
     def __init__(self, ftx_comparable_contract, strike_threshold=200, days_threshold=1):
        super().__init__()
        """
     ...
     def get_comparable_deribit(self):
        ...
        df = df.sort_values("strike").reset_index().drop("index", axis=1)
        return df
     ...
     def compare(self, data):
        ...
        straddle_instruments = data.instrument_name.values
        ...
        data = { 'deribit_straddle': [straddle_instruments, straddle_price, straddle_time],
